...
io.on('connection', (socket) => {
    ...
    socket.on('playerMove', ({ roomId, choice, isSpecial }) => {
        const room = rooms[roomId];
        if (!room || room.gameState.status !== 'playing' || !room.players || room.players.length !== 2) { console.warn(`Move ignored: Room ${roomId} invalid.`); return; }
...
function resolveTurn(roomId) {
    ...
    if (p1.hp <= 0 || p2.hp <= 0) { finishGame(roomId); return; }
    ...
    if (p1.hp <= 0 || p2.hp <= 0) {
        ...
        // Send updated state including turn results for animation
...
function resolveCombat(roomId, p1_choice, p2_choice) {
    ...
    if (rps_result === 'tie') {
        state.clashMultiplier++; result_text = `CLASH! Next round is worth ${state.clashMultiplier}x!`; isClash = true;
        if (p1.consecutiveWins > 0 || p1.consecutiveLosses > 0 || p2.consecutiveWins > 0 || p2.consecutiveLosses > 0) special_text += " Streak broken!";
        p1.consecutiveWins = 0; p1.consecutiveLosses = 0; p2.consecutiveWins = 0; p2.consecutiveLosses = 0;
    } else {
        let charge = 0, penalty = 0, hp_swing = 0;
        let winnerPlayer, loserPlayer, winnerIndex;
        if (rps_result === 'win') { winnerPlayer = p1; loserPlayer = p2; winnerIndex = 0; }
        else { winnerPlayer = p2; loserPlayer = p1; winnerIndex = 1; }
        loserPlayer.consecutiveWins = 0; winnerPlayer.consecutiveWins++; loserPlayer.consecutiveLosses++;
        const winnerElemResult = winnerIndex === 0 ? elem_result : (elem_result === 'win' ? 'lose' : (elem_result === 'lose' ? 'win' : 'tie'));

        if (winnerElemResult === 'win') { hp_swing = CRIT_DAMAGE; charge = CRIT_SPECIAL_CHARGE; penalty = BASE_SPECIAL_LOSS * 2; result_text = `Player ${winnerIndex + 1} CRITICAL HIT!`; }
        else if (winnerElemResult === 'tie') { hp_swing = NORMAL_DAMAGE; charge = calculateStreakCharge(winnerPlayer.consecutiveWins); penalty = calculateStreakPenalty(loserPlayer.consecutiveLosses); result_text = `Player ${winnerIndex + 1} wins!`; }
        else { hp_swing = RESIST_DAMAGE; charge = 10; penalty = 0; winnerPlayer.consecutiveWins = 0; loserPlayer.consecutiveLosses = 0; result_text = `Player ${winnerIndex + 1}'s hit was DAMPENED!`; }

        charge = Math.round(charge * state.clashMultiplier); penalty = Math.round(penalty * state.clashMultiplier); hp_swing = Math.round(hp_swing * state.clashMultiplier);
        if (winnerIndex === 0) { p1_heal = hp_swing; p2_damage = hp_swing; p1.special = Math.min(100, p1.special + charge); p2.special = Math.max(0, p2.special - penalty); special_text = `P1 +${charge} Sp! (P2 -${penalty} Sp)`; if(p1.consecutiveWins > 1 && winnerElemResult !== 'lose') special_text += ` (${p1.consecutiveWins}x win!)`; }
        else { p2_heal = hp_swing; p1_damage = hp_swing; p2.special = Math.min(100, p2.special + charge); p1.special = Math.max(0, p1.special - penalty); special_text = `P1 -${penalty} Sp! (P2 +${charge} Sp)`; if(p2.consecutiveWins > 1 && winnerElemResult !== 'win') special_text += ` (${p2.consecutiveWins}x win!)`; }
        state.clashMultiplier = 1; result_text += ` (±${hp_swing} HP)`;
    }
    p1.hp = Math.min(p1.maxHp, Math.max(0, p1.hp - p1_damage + p1_heal));
    p2.hp = Math.min(p2.maxHp, Math.max(0, p2.hp - p2_damage + p2_heal));
    state.roundResultText = result_text.trim(); state.specialResultText = special_text.trim();
    console.log(`Combat resolved in ${roomId}. P1 HP:${p1.hp}, Sp:${p1.special} | P2 HP:${p2.hp}, Sp:${p2.special}`);
...
function getElementResult(p1_elem, p2_elem, roomId) { const room = rooms[roomId]; if (!room || !room.gameState || !room.gameState.elementalTriangle) return 'tie'; const triangle = room.gameState.elementalTriangle; if (!p1_elem || !p2_elem) return 'tie'; if (p1_elem === p2_elem) return 'tie'; if (triangle[p1_elem] === p2_elem) return 'win'; return 'lose'; }
...
function shuffleGambits(roomId, playerId) { const room = rooms[roomId]; if (!room || !room.players) return; const playerIndex = room.players.findIndex(p => p.id === playerId); if (playerIndex === -1) return; let shuffledGambits = [...GAMBIT_KEYS]; for (let i = shuffledGambits.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [shuffledGambits[i], shuffledGambits[j]] = [shuffledGambits[j], shuffledGambits[i]]; } room.players[playerIndex].gambitAssignments = { 'rock': shuffledGambits[0], 'paper': shuffledGambits[1], 'scissors': shuffledGambits[2] }; console.log(`Shuffled gambits for P${playerIndex+1} (${playerId}) in ${roomId}`); }

function finishGame(roomId) {
    const room = rooms[roomId];
    if (!room || room.gameState.status === 'finished') return;
    room.gameState.status = 'finished';
    const p1 = room.players[0]; const p2 = room.players.length > 1 ? room.players[1] : null;
    let winnerId = null;
    if (p1 && p2) { if (p1.hp <= 0 && p2.hp <= 0) winnerId = null; else if (p2.hp <= 0) winnerId = p1.id; else if (p1.hp <= 0) winnerId = p2.id; }
    else if (p1 && p1.hp > 0) { winnerId = p1.id; console.log(`Game in ${roomId} ended with only P1.`); }
    else { console.log(`Game in ${roomId} ended inconclusively.`); }

     const finalPlayersState = room.players.map(p => ({ id: p.id, hp: p.hp, special: p.special, maxHp: p.maxHp }));
     io.to(roomId).emit('gameStateUpdate', { players: finalPlayersState, gameState: { clashMultiplier: room.gameState.clashMultiplier, roundResultText: room.gameState.roundResultText, specialResultText: room.gameState.specialResultText }, turnResult: null, gameOver: true });
     setTimeout(() => {
        if (rooms[roomId]) { io.to(roomId).emit('gameOver', { winnerId }); console.log(`Game over in room ${roomId}. Winner: ${winnerId === null ? 'Draw' : winnerId}.`); }
        // Optional cleanup: setTimeout(() => { if (rooms[roomId]) delete rooms[roomId]; }, 10000);
     }, 100);
}
